...
const Mnemonic = require('ltzcore-mnemonic');
const { PrivateKey } = require('ltzcore-lib');
...
export class Wallet {
  ...
  static async create(params: Partial<Wallet.WalletObj>) {
    ...
    // save wallet to storage and then ltzcore-node
    ...
    await loadedWallet.register().catch(e => {
      console.debug(e);
      console.error('Failed to register wallet with ltzcore-node.');
    });
    return loadedWallet;
  }
