...
//console.log('[paypro.js.21]', k,v); //TODO
  ...
  it('Make a PP request with browser', function(done) {
    ...
    PayPro.get({
      ...
      headers['Accept'].should.equal('application/litecoinz-paymentrequest');
  ...
  it('Create a PP payment', function() {
    ...
    var payment = PayPro._createPayment(data.merchant_data, '12ab1234', 'mwRGmB4NE3bG4EbXJKTHf8uvodoUtMCRhZ', 100, 'ltz');
    var s = '';
  ...
  it('Send a PP payment (browser, LTZ)', function(done) {
    var data = TestData.payProData;
    var opts = {
    ...
    var payment = PayPro.send(opts, function(err, data) {
      headers['Accept'].should.equal('application/litecoinz-paymentack');
      headers['Content-Type'].should.equal('application/litecoinz-payment');
      should.not.exist(err);
      done();
    });
  });
