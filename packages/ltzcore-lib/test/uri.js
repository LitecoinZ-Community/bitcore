...
describe('URI', function() {
  ...
  it('parses uri strings correctly (test vector)', function() {
    ...
    uri = URI.parse('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    uri.address.should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    expect(uri.amount).to.be.equal(undefined);
    expect(uri.otherParam).to.be.equal(undefined);

    uri = URI.parse('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=123.22');
    uri.address.should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  it('URIs can be validated statically (test vector)', function() {
    URI.isValid('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj').should.equal(true);
    URI.isValid('litecoinz:mkYY5NRvikVBY1EPtaq9fAFgquesdjqECw').should.equal(true);

    URI.isValid('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2')
                .should.equal(true);
    URI.isValid('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param')
                .should.equal(true);
    ...
    URI.isValid('litecoinz:mmrqEBJxUCf42vdb3oozZtyz5mKr3Vb2Em?amount=0.1&' +
                'r=https%3A%2F%2Ftest.litecoinz.org%2Fi%2F6DKgf8cnJC388irbXk5hHu').should.equal(true);
    ...
    URI.isValid('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=1.2&req-other=param')
                .should.equal(false);
    URI.isValid('litecoinz:?r=https%3A%2F%2Ftest.litecoinz.org%2Fi%2F6DKgf8cnJC388irbXk5hHu')
                .should.equal(false);
  });
  ...
  it('do not need new keyword', function() {
    var uri = URI('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  describe('instantiation from litecoinz uri', function() {
    ...
    it('parses address', function() {
      uri = new URI('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.livenet);
    });

    it('parses amount', function() {
      uri = URI.fromString('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=123.22');
    ...
    it('parses a testnet address', function() {
      uri = new URI('litecoinz:mkYY5NRvikVBY1EPtaq9fAFgquesdjqECw');
      uri.address.should.be.instanceof(bitcore.Address);
      uri.network.should.equal(Networks.testnet);
    });

    it('stores unknown parameters as "extras"', function() {
      uri = new URI('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param');
      uri.address.should.be.instanceof(bitcore.Address);
    ...
    it('throws error when a required feature is not supported', function() {
      (function() {
        return new URI('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param&req-required=param');
      }).should.throw(Error);
    });
  ...
  it('should support double slash scheme', function() {
    var uri = new URI('litecoinz://1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    uri.address.toString().should.equal('1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  });

  it('should input/output String', function() {
    var str = 'litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?' +
  ...
  it('should support numeric amounts', function() {
    var uri = new URI('litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=12.10001');
  ...
  it('should generate a valid URI', function() {
    new URI({
      ...
      'litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj'
    );
    ...
    new URI({
      ...
      'litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.10001&message=Hello%20World&something=else'
    );
  ...
  it('writes correctly the "r" parameter on string serialization', function() {
    var originalString = 'litecoinz:mmrqEBJxUCf42vdb3oozZtyz5mKr3Vb2Em?amount=0.1&' +
                         'r=https%3A%2F%2Ftest.litecoinz.org%2Fi%2F6DKgf8cnJC388irbXk5hHu';
    var uri = new URI(originalString);
    uri.toString().should.equal(originalString);
  });

  it('displays nicely on the console (#inspect)', function() {
    var uri = 'litecoinz:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj';
